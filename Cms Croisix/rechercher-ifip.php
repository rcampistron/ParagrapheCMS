...
?>
	<div class="item" id="coltexte785"> 
		<div class="sap-content">
			<p class="titre785">Résultats de la recherche pour le mot "<?=stripslashes($rech_site)?>" </p>
			<table width="760">
					<tr>
						<td colspan="3">
				<?php
			  ...
			  <?php
				...
				foreach ($output as $key => $value) {
					...
					<tr>
						<td>
						<img src="images/picto-pdf-acces-reserve.gif" alt="Rapport en accès réservé" width="15" height="14" />
						</td>
						<td>&nbsp;</td>
						<td>
						<p class="marge10"><a href="javascript:if (confirm('Ce document est en accès réservé. Souhaitez-vous être redirigé vers l\'espace Pro ?')) window.location='index.php?numpage=<?=$pagePro->numpage?>&spec=<?=$pagePro->nomFichier?>&numrub=<?=$pagePro->numrub?>&numcateg=<?=$pagePro->numcateg?>&numsscateg=<?=$pagePro->numsscateg?>&numpara=<?=$value["numpara"]?>&lg=<?=$lg?>'"> <?=$value["titre"]?></a></p>
						</td>
					</tr>
					<?php
					...
					<tr>
					   ...
					   <p class="marge10"><?=$value["texte"]?><br />
						<? if ($value["auteurs"]) { ?>
						<span class="petit"><strong>Auteurs : </strong><?=$value["auteurs"]?></span><br /><? } ?>
						<span class="petit"><strong>Mots-clés : </strong><?=str_replace(";",", ",$value["keyw"])?></span>
						</p>
						</td>
					</tr>
					<?php
					...
					<tr>
						...
						<td>&nbsp;</td>
						<td>
						<p><a href="<?=$value["lien"]?>" target="_blank"> <?=$value["titre"]?> (<?=$value["poids_fiche"]?>)</a></p>
					   <p class="marge10"><?=$value["texte"]?><br />
						<? if ($value["auteurs"]) { ?>
						<span class="petit"><strong>Auteurs : </strong><?=$value["auteurs"]?></span><br /><? } ?>
						<span class="petit"><strong>Mots-clés : </strong><?=str_replace(";",", ",$value["keyw"])?></span>
					...
					<tr>
						...
						<td>&nbsp;</td>
						<td>
						<p><a href="index.php?numpage=<?=$pageBddDoc->numpage?>&spec=<?=$pageBddDoc->nomFichier?>&numrub=<?=$pageBddDoc->numrub?>&numcateg=<?=$pageBddDoc->numcateg?>&numsscateg=<?=$pageBddDoc->numsscateg?>&lg=<?=$lg?>&numpara=<?=$value["numpara"]?>"> <?=$value["titre"]?> (<?=$value["poids_fiche"]?>)</a></p>
					   <p class="marge10"><?=$value["texte"]?><br />
						<? if ($value["auteurs"]) { ?>
						<span class="petit"><strong>Auteurs : </strong><?=$value["auteurs"]?></span><br /><? } ?>
						<span class="petit"><strong>Mots-clés : </strong><?=str_replace(";",", ",$value["keyw"])?></span>
   ...
   <input type="hidden" name="rech_site_suite" id="rech_site_suite" value="<?=$rech_site?>" />
...
?>
	<div class="item" id="coltexte785"> 
		<div class="sap-content">
			<p class="titre785">Il n'y a aucun résultat pour le mot "<?=stripslashes($rech_site)?>" </p>
			<p>&nbsp;</p>
			<p>&nbsp;</p>
			<p>&nbsp;</p>
			<p>&nbsp;</p>

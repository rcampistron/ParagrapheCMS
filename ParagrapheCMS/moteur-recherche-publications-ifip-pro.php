<?php /* Date de création: 17/07/2009 */ 
// Il faut finalement un moteur de recherche specifique PRO
// du fait de la complxite dans la gestion amont/aval etc.
...
<table width="<? if ($spec=="extranet-pro") echo"785"; else echo"550";?>" border="0" cellspacing="0" cellpadding="0">
                  ...
                  <tr>
				   ...
				   <option value=""></option>  
				   <?php
				   $numr=SelectSimple("numrub","if_rubrique","1","1"," AND nom LIKE 'domaines d\'Expertise'");//numero de la rubrique correspondant  "Domaine d'expertise"
			...
			<div id="suggestion"></div> 
			 <p>&nbsp;<br />
					...
					$dernieresParu->doc=1;
					$req_doc=" WHERE  publiee='o' AND acces_res!='o' ORDER BY date DESC LIMIT 0,4";
				    if ($spec=="extranet-pro") $req_doc=" WHERE  publiee='o' AND acces_res='o' ORDER BY date DESC LIMIT 0,3";
...
if ($dom) {
	...
	if ($annee) {//on a sélectionné un critère de date
		...
		$tri_date=" AND publiee='o' AND (date BETWEEN $date_deb AND $date_fin)";
	}
					
	if ($type_doc) {//on a sélectionné un type de document
		$tri_date.=" AND publiee='o' AND type_doc='$type_doc'";
	}
					
	if ($spec=="extranet-pro") $tri_date.=" AND publiee='o' AND acces_res='o'";
	...
	if ($ssdom) {//on a sélectionné un sous-domaine
		...
		$total=count($menus_doc->listdoc); //car $nb_doc_menu de afficherDocsSousCateg() renvoi juste true ou false et listdoc est incrémenté dans afficherDocsSousCateg()
					...
					$req_doc.=" ORDER BY date DESC"; //remise en service (plus de tri sur annee sur le tableau des objets)
						...
						if ($total) {
				  		...
				  		?>
							<tr>
							  ...
							  <strong>
							 <?php
								echo $total." document(s) réservés Pro";
								echo"<br />";
								$de=$s+1;
